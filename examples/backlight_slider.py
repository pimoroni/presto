...
backlight_max_icon = shape.custom(*[[vec2(x, y) for x, y in path] for path in BACKLIGHT_MAX_PATHS])
backlight_min_icon = shape.custom(*[[vec2(x, y) for x, y in path] for path in BACKLIGHT_MIN_PATHS])
backlight_max_icon.transform = mat3().translate(CX, BAR_Y + 20)
backlight_min_icon.transform = mat3().translate(CX, BAR_Y + BAR_H - 20)
...
class Slider(object):
    def __init__(self, x, y, w, h, bar_colour, slider_colour):
        ...
        self.value = 1.0
        self.slider_vector = shape.rounded_rectangle(self.x, self.y, self.w, self.slider_h, 10)
        self.bar_vector = shape.rounded_rectangle(self.x, self.y, self.w, self.h, 10)
    ...
    def update(self):
        ...
        if self.touch.state and self.touch.x >= self.x and self.touch.x <= self.x + self.w and self.touch.y >= self.y - self.slider_h and self.touch.y <= (self.y + self.h):
            ...
            self.slider_vector = shape.rounded_rectangle(self.x, new_pos_y, self.w, self.slider_h, 10)
            self.value = 1.0 - (new_pos_y - self.y) / ((self.y + self.h - self.slider_h) - self.y)

    def draw(self):
        display.pen = self.bar_colour
        display.shape(self.bar_vector)
        display.pen = self.slider_colour
        display.shape(self.slider_vector)
...
while True:
    ...
    display.shape(backlight_max_icon)
    display.shape(backlight_min_icon)
